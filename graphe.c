...
Graphe create_grapheReduit(Graphe g){

	Cell b=_createCell(BLANC);
	Cell n=_createCell(NOIR);
	for(int i=0;i<g->size_groupsW;i++){
		for(int j=0;g->groupsW[i][j]!=NULL;j++){
			for(int h=0;h<6;h++){
				if(g->groupsW[i][j]->cells[h]->key!=BLANC && g->groupsW[i][j]->cells[h]->key!='W' && g->groupsW[i][j]->cells[h]->key!='B' )
					g->groupsW[i][j]->cells[h]->cells[(h+3)%6]=b;
			}
	...
	for(int i=0;i<g->size_groupsB;i++){
		for(int j=0;g->groupsB[i][j]!=NULL;j++){
			for(int h=0;h<6;h++){
				if(g->groupsB[i][j]->cells[h]->key!=NOIR && g->groupsW[i][j]->cells[h]->key!='B' && g->groupsW[i][j]->cells[h]->key!='B')
					g->groupsB[i][j]->cells[h]->cells[(h+3)%6]=n;
